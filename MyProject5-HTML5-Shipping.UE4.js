...
var Module = {
	...
	UE4_indexedDBVersion: 202404161033, // Bump this number to invalidate existing IDB storages in browsers.
};
...
$(document).ready(function() {
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('MyProject5-HTML5-Shipping.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('MyProject5-HTML5-Shipping.js'), 'blob').then(function(data) {
